...
class loueq_loop():
    ...
    def __init__(self, links, run=100):
        ...
        self.nodes, self.fitness_dict=MaxEntCM(self.links)
...
class loueq():
    ...
    def step_0(self, nodes):
        ...
        # PAY ATTENTION! Actually, I sould not consider the nn only for getting the 
        ...
        for _new_memb in nn_memb:
            ...
            for node in nodes:
                ...
                x_i=self.fitness_dict[node]
                a_i=self.adl[node]
                _deltaQ+=self.delta_Q_0(new_comm, a_i, x_i)
            if _deltaQ>deltaQ:
                deltaQ=_deltaQ
                new_memb=_new_memb
...
def MaxEntCM(links):
    ...
    nodes=list(myGraph.nodes_dict.values())
    fitness_dict=dict(zip(nodes, myGraph.x))
    
    return nodes, fitness_dict
